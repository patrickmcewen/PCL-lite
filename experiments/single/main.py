...
def parallel_affine_impl(model_name, base_path, example_path, task_path, exp_name, num_samples, temperature=0.7, max_tokens=1024):
    ...
    # Store the usage
    temp_dir = config.retrieve(copy_key)["temp_dir"]
    usage = storage.retrieve(copy_key)["usage"]
    with open(os.path.join(temp_dir, "usage.json"), "w") as f:
        json.dump(usage, f, indent=4)
    stats = {}
    stats['success'] = len(storage.retrieve(copy_key)['success'])
    stats['success_rep'] = len(storage.retrieve(copy_key)['success_rep'])
    stats['failure'] = len(storage.retrieve(copy_key)['failure'])
    stats['failure_rep'] = len(storage.retrieve(copy_key)['failure_rep'])
