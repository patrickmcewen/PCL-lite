...
if __name__ == "__main__":
    ...
    # gpt-oss-120b at reasoning_effort=high needs a much larger budget than
    # the DeepSeek-era 1024-token default; CoT eats the budget otherwise.
    parser.add_argument("--max_tokens", type=int, default=1024)
    # Reference benchcard used by merge_best.keep_best_success to know the
    # full task set. Default keeps backwards compatibility with the old
    # hard-coded path; pass the StepDB smoke csv (or any other benchcard)
    # when running outside the legacy PCL-lite suite.
    ...
    ref_file = args.ref_csv or f"{project_path}/experiments/benchcard_fullpath.csv"
    assert os.path.exists(ref_file), f"ref_csv {ref_file} not found"
    ...
    while True:
        ...
        for m in ms:
            ...
            args.example_path = prompt_with_examples_file
